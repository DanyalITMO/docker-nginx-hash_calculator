...
docker run -d -p 127.0.0.1:8081:80 --cpus=0.125 -e "WORKER_COUNT=1" -e "MEM_SIZE=100000000" -v $path_to_data:/files --name=nginx_media1 nginx/hash_calculator
docker run -d -p 127.0.0.2:8081:80 --cpus=0.19 -e "WORKER_COUNT=2" -e "MEM_SIZE=200000000" -v $path_to_data:/files --name=nginx_media2 nginx/hash_calculator
docker run -d -p 127.0.0.3:8081:80 --cpus=0.3 -e "WORKER_COUNT=3" -e "MEM_SIZE=300000000" -v $path_to_data:/files --name=nginx_media3 nginx/hash_calculator
