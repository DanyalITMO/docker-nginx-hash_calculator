...
docker run -d -p 127.0.0.2:8081:80 --cpus=0.5 -e "WORKER_COUNT=1" -e "MEM_SIZE=1000000000" -v $path_to_data:/files --name=nginx_media1 nginx/hash_calculator
docker run -d -p 127.0.0.3:8081:80 --cpus=0.75 -e "WORKER_COUNT=2" -e "MEM_SIZE=2000000000" -v $path_to_data:/files --name=nginx_media2 nginx/hash_calculator
docker run -d -p 127.0.0.4:8081:80 --cpus=1 -e "WORKER_COUNT=3" -e "MEM_SIZE=3000000000" -v $path_to_data:/files --name=nginx_media3 nginx/hash_calculator
